...
def generate_measurement_equations(
    ...
    :param car_centric_attitude: expression for the latent variable.
    :param urban_preference_attitude: expression for the latent variable.
    :return: a dict associating each level of the Liker scale with the Expression
        calculating the corresponding probability.

    The first one is normalized to -1. Indeed, we expect the respondents with a higher
    car centric attitude to disagree more with the statement
    "Fuel price should be increased to reduce congestion and air pollution."
    (corresponding to a low value of the indicator) compared to those who have a
    lower car centric attitude. In other words, when the car centric attitude
    increases, we expect the value of the indicator to decrease.
    """
    models = {
        k: intercepts[k]
    ...
    dict_prob_indicators = {
        indicator: {
            ...
            - NormalCdf(
                tau_1_residual[indicator],
            ),
            3: NormalCdf(tau_3_residual[indicator])
            - NormalCdf(
                tau_2_residual[indicator],
            ),
            4: NormalCdf(tau_4_residual[indicator])
            - NormalCdf(
                tau_3_residual[indicator],
            ),
