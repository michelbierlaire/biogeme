...
def read_data() -> db.Database:
    ...
    _ = database.DefineVariable('SM_COST', SM_CO * (GA == 0))
    _ = database.DefineVariable('TRAIN_COST', TRAIN_CO * (GA == 0))
    _ = database.DefineVariable('CAR_AV_SP', CAR_AV * (SP != 0))
    _ = database.DefineVariable('TRAIN_AV_SP', TRAIN_AV * (SP != 0))
    _ = database.DefineVariable('TRAIN_TT_SCALED', TRAIN_TT / 100)
    _ = database.DefineVariable('TRAIN_COST_SCALED', TRAIN_COST / 100)
    _ = database.DefineVariable('SM_TT_SCALED', SM_TT / 100)
