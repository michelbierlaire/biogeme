...
def read_data() -> db.Database:
    ...
    df.drop(df[df["number_of_cars"] == -1].index, inplace=True)
    ...
    _ = database.define_variable('used_to_go_to_school_by_car', ModeToSchool == 1)
    _ = database.define_variable('city_center_as_kid', ResidChild == 1)
    ...
    # % 30% / 60% quantile of the distance in the data
    _ = database.define_variable('distance_category', 1 + (distance_km >= 50))
    ...
    _ = database.define_variable('high_education', Education >= 6)
    _ = database.define_variable('low_education', Education <= 3)
    _ = database.define_variable('top_manager', SocioProfCat == 1)
    ...
    _ = database.define_variable('car_oriented_parents', FreqCarPar == 4)
