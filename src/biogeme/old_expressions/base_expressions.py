...
from jax import jit
from jax._src.errors import TracerIntegerConversionError
...
from .jax import build_vectorized_function, JAX_FLOAT
...
class Expression:
    ...
    def get_value_and_derivatives_jax(
        self,
        ...
        """
        Calculates the value of the expression, and its derivatives using biogeme_jax.
        :return: the function takes two parameters: the parameters, and one row of the database.
        """
        if not gradient and (hessian or bhhh):
            raise ValueError(
                'If you need second order derivatives, you also need first order.'
            )

        if not betas and gradient:
            raise BiogemeError(
                'The gradient is requested but no parameter is provided.'
            )

        if gradient:
            betas_in_expression = self.dict_of_elementary_expression(
        ...
        the_function = self._recursive_construct_jax_function()
    ...
    def dict_of_catalogs(self, ignore_synchronized: bool = False) -> dict[str, Catalog]:
