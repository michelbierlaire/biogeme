...
class BIOGEME:
    ...
    def calculate_likelihood_and_derivatives(
        ...
        :raises BiogemeError: if calculation with batch is requested
        """
        return self.calculate_likelihood_and_derivatives_cython(
            x=x, scaled=scaled, hessian=hessian, bhhh=bhhh, batch=batch
        )

    def calculate_likelihood_and_derivatives_cython(
        self,
        x: np.ndarray | list[float],
        scaled: bool,
        hessian: bool = False,
