...
class Database:
    ...
    def suggestScaling(self, columns = None):
        ...
        largestValue = [max(np.abs(self.data[col].max()),np.abs(self.data[col].min())) for col in columns]
        res = [[col, 1 / 10**np.round(np.log10(max(1.0, l))), l] for col, l in zip(self.data.columns, largestValue)]
    ...
    def split(self, slices):
        ...
        theSlices = np.array_split(self.data, slices)
        estimationSets = []
        validationSets = []
        for i in range(len(theSlices)):
            estimationSets.append(pd.concat(theSlices[:i]+theSlices[i+1:]))
            validationSets.append(theSlices[i])
