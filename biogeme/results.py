...
class beta:
    ...
    def isBoundActive(self,threshold=1.0e-6):
        ...
        :param threshold: distance below which the bound is considered to be active. Default: :math:`10^{-6}`
...
class bioResults:
    ...
    def __init__(self,rawResults=None,pickleFile=None):
        ...
        :param rawResults: object with the results of the estimation. Default: None.
        :type rawResults: biogeme.results.rawResults 
        :param pickleFile: name of the file containing the raw results in pickle format. Default: None.
    ...
    def _calculateStats(self):
        """Calculates the following statistics:
            - likelihood ratio test between the initial and ethe estimated models: \f[-2(L_0-L^*) \f]
            - Rho square: \f[ 1 - \frac{L^*}{L^0}\f]
            - Rho bar square: \f[ 1 - \frac{L^* - K}{L^0}\f]
            - AIC: \f[2(K - L^*) \f]
            - BIC: \f[ -2 L^* + K  \log(N) \f]
        ...
        if self.data.bootstrap is not None:
            ...
            if (rd > 0).all():
                ...
                self.data.bootstrap_correlation = np.full_like(self.data.bootstrap_varCovar,np.finfo(float).max)
...
a Pandas dataframe.
        ...
        for b in self.data.betas:
            if b.isBoundActive():
                anyActiveBound = True
        if anyActiveBound:
            columns = ['Value','Active bound','Std err','t-test','p-value','Rob. Std err','Rob. t-test','Rob. p-value']
        else:
...
#                print(f"values[{k}] = {v}")
        ...
        if np.abs(self.data.smallestEigenValue) <= 1.0e-5:
            ...
            for i in range(len(self.data.smallestEigenVector)):
                if np.abs(self.data.smallestEigenVector[i]) > 1.0e-5:
                    h += f'<tr><td>{self.data.smallestEigenVector[i]:.3g}</td><td> *</td><td> {self.data.betaNames[i]}</td></tr>\n'
            h += "</table>"
            h += '</p>\n'
                
        h += "<p>Smallest singular value: {:.6g}</p>\n".format(self.data.smallestSingularValue)
        h += self._getHtmlFooter()
        return h
        ...
        for b in myBetas:
            try:
                ...
                for k in self.data.betaNames:
                    keys += f' {k}'
                err = f"The value of {b} is not available in the results. The following parameters are available: {keys}"
    ...
    def getBootstrapVarCovar(self):
        ...
        for i in range(len(self.data.betas)):
            for j in range(len(self.data.betas)):
                vc.at[self.data.betas[i].name, self.data.betas[j].name] = self.data.bootstrap_varCovar[i,j]
        return vc   
    ...
    def _getHtmlFooter(self):
        """Prepare the footer for the HTML file.

        Return:
           string containing the footer.
        """
        return "</html>"
    ...
    def getBetasForSensitivityAnalysis(self,myBetas,size=100,useBootstrap=False):
        ...
        :raise biogeme.exceptions.biogemeError: if useBootstrap is True and the bootstrap matrix is not available.
        ...
        if useBootstrap and self.data.bootstrap is None:
            err = f"Bootstrap variance-covariance matrix not available for simulation. Use useBootstrap=False."
            raise excep.biogemeError(err)
            

        theMatrix = self.data.bootstrap_varCovar if useBootstrap else self.data.robust_varCovar
        simulatedBetas = np.random.multivariate_normal(self.data.betaValues, theMatrix, size, check_valid='warn')
