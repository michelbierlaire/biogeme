...
class smoothing:
    ...
    def add(self, f, g, h, batch, discount = 0.95):
        if g is not None:
            if self.n is None:
                ...
                raise excep.biogemeError(f'Incompatible dimensions {len(g)} and {self.n}')
        if h is not None:
            if h.shape != (self.n,self.n):
                raise excep.biogemeError(f'Incompatible dimensions {h.shape} and ({self.n},{self.n})')
    ...
    def f_g_h(self,discount = 0.95):
        ...
        scale = [self.batch[-1]] + [discount ** k * (1.0-self.batch[-1]) / (self.windowSize - 1) for k in range(1,min(len(self.f),self.windowSize))]
...
#    stats = pd.DataFrame(columns=columns)
    ...
    while cont:
        ...
        k += 1
        if batch <= hybrid:
            success, xc, fc, gc, hc, delta = generateCandidateSecondOrder(fct, xk, avgf, avgg, avgh, batch, delta, dogleg, maxFailure, maxDelta, eta1, eta2)
        else:
            success, xc, fc, gc, hc, delta = generateCandidateFirstOrder(fct, xk, avgf, avgg, avgh, batch, delta, dogleg, maxFailure, maxDelta, eta1, eta2)
