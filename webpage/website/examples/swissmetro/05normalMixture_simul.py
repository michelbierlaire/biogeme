...
try:
    ...
    print('Run first the script 05normalMixture.py in order to generate the file '
...
simresults['right'] = np.log(simresults['Integral'] + 1.96 * simresults['Integration error'])

print(f'Log likelihood: {np.log(simresults["Integral"]).sum()}')
print(f'Integration error for {numberOfDraws} draws: {simresults["Integration error"].sum()}')
print(f'In average {simresults["Integration error"].mean()} per observation.')
print(f'95% confidence interval: [{simresults["left"].sum()}-{simresults["right"].sum()}]')
