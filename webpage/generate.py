...
def copyFile(name, adir='sources'):
    orig = f'{adir}/{name}'
    dest = f'website/{name}'
    shutil.copy(orig, dest)


def copyExample(name, adir, hfiles):
...
def generateMenu(items):
    ...
    with open(file, 'w') as f:
        ...
        for ff, tt in items.items():
            print(
                f'<li class=\'biomenu\'>'
                f'<a class=\'{ff}ACTIVE\' href=\'{ff}.html\'>{tt}</a></li>',
                file=f,
            )
        print('</ul>', file=f)
...
def oneExample(name, dir, all, html):
    ...
    for i, doc in files:
        ...
        result += '<td>'
        result += (
            f'<a href="{urlgithub}examples/{dir}/{main}.ipynb" '
            f'target="_blank">'
            f'<img src="github.png" height="30"></a>&nbsp;'
        )
        result += (
            f'<a href="{urlnoto}examples/{dir}/{main}.ipynb" target="_blank">'
            f'<img src="jupyter.png" height="30"></a>&nbsp;'
        )
        result += (
            f'<a href="{urlbinder}examples/{dir}/{main}.ipynb" '
            f'target="_blank">'
