...
from biogeme.bayesian_estimation import RawBayesianResults
from biogeme.bayesian_estimation import bayesian_results as br_mod
...
class DummyRawBayesianResults:
    ...
    def __init__(
        ...
        # Infer number of observations from the log-likelihood group if present.
        ll = (
            idata.log_likelihood.get(CHOICE_LABEL, None)
...
def simple_idata() -> az.InferenceData:
    ...
    chains, draws, obs, arr_dim = 4, 20, 7, 4

    # Posterior scalars. The number of draws is intentionally large enough for
    # PSIS-LOO, which needs at least five tail draws with recent ArviZ versions.
...
def dummy_raw(simple_idata: az.InferenceData) -> RawBayesianResults:
    ...
    # Infer number_of_observations from the log_likelihood group.
    log_like = simple_idata.log_likelihood[CHOICE_LABEL]
...
def test_log_likelihood_property_adds_group(bayes_results: BayesianResults) -> None:
    """Accessing log_likelihood should expose the log_likelihood group with CHOICE_LABEL."""
...
def test_waic_is_disabled_in_default_fixture(bayes_results: BayesianResults) -> None:
    """WAIC is disabled in the default fixture because ArviZ no longer provides it."""

    assert bayes_results.waic is None
    assert bayes_results.waic_se is None
    assert bayes_results.p_waic is None


def test_waic_returns_none_when_unavailable(
    dummy_raw: RawBayesianResults,
    caplog: pytest.LogCaptureFixture,
) -> None:
    """Requesting WAIC should warn and return None if ArviZ does not provide it."""
    ...
    if hasattr(az, 'waic'):
        assert results.waic is not None
    else:
        with caplog.at_level('WARNING'):
            assert results.waic is None
        assert 'WAIC requested' in caplog.text
        assert 'does not provide az.waic' in caplog.text


def test_loo_properties(bayes_results: BayesianResults) -> None:
    """LOO and its derived quantities should be finite when enabled."""
...
def test_loo_returns_none_when_arviz_raises_value_error(
    bayes_results: BayesianResults,
...
def test_general_information_omits_loo_when_arviz_raises_value_error(
    bayes_results: BayesianResults,
    monkeypatch: pytest.MonkeyPatch,
    caplog: pytest.LogCaptureFixture,
) -> None:
    """General information should be generated even when LOO is unavailable."""
    ...
    assert 'LOO (Leave-One-Out Cross-Validation)' not in info
    assert 'LOO requested' in caplog.text
...
def test_to_summary_survives_loo_failure(
    bayes_results: BayesianResults,
    monkeypatch: pytest.MonkeyPatch,
    caplog: pytest.LogCaptureFixture,
) -> None:
    """to_summary should still return a summary when LOO cannot be computed."""
    ...
    assert summary.loo is None
    assert summary.loo_se is None
    assert summary.p_loo is None
    assert 'LOO requested' in caplog.text
...
def test_generate_general_information_and_short_summary(
    ...
    assert 'WAIC (Widely Applicable Information Criterion)' not in info
