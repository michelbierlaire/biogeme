...
class TestUniformPdf(unittest.TestCase):
    def test_within_bounds(self):
        ...
        self.assertAlmostEqual(uniformpdf(0, -1, 1).getValue(), 0.5)
        self.assertAlmostEqual(uniformpdf(0.5, -1, 1).getValue(), 0.5)

    def test_outside_bounds(self):
        # Test with x outside the bounds [a, b]
        self.assertAlmostEqual(uniformpdf(-2, -1, 1).getValue(), 0.0)
        self.assertAlmostEqual(uniformpdf(2, -1, 1).getValue(), 0.0)

    def test_expression_inputs(self):
        self.assertAlmostEqual(
            uniformpdf(Numeric(0), Numeric(-1), Numeric(1)).getValue(), 0.5
        )
        self.assertAlmostEqual(
            uniformpdf(Numeric(0.5), Numeric(-1), Numeric(1)).getValue(), 0.5
        )
    ...
    def test_mixed_inputs(self):
        # Test with mixed type inputs
        self.assertAlmostEqual(uniformpdf(0.5, Numeric(-1), 1).getValue(), 0.5)

    def test_edge_cases(self):
        # Test edge cases (x at the boundaries a and b)
        self.assertAlmostEqual(uniformpdf(-1, -1, 1).getValue(), 0.5)
        self.assertAlmostEqual(uniformpdf(1, -1, 1).getValue(), 0.5)
...
class TestTriangularPdf(unittest.TestCase):
    def test_valid_values(self):
        ...
        self.assertAlmostEqual(triangularpdf(0, -1, 1, 0).getValue(), 1.0)
        self.assertAlmostEqual(triangularpdf(-0.5, -1, 1, 0).getValue(), 0.5)
        self.assertAlmostEqual(triangularpdf(0.5, -1, 1, 0).getValue(), 0.5)

    def test_outside_bounds(self):
        # Test with x outside the bounds [a, b]
        self.assertAlmostEqual(triangularpdf(-2, -1, 1, 0).getValue(), 0.0)
        self.assertAlmostEqual(triangularpdf(2, -1, 1, 0).getValue(), 0.0)
    ...
    def test_expression_inputs(self):
        ...
        self.assertAlmostEqual(
            triangularpdf(Numeric(0), Numeric(-1), Numeric(1), Numeric(0)).getValue(),
            1.0,
        )
        self.assertAlmostEqual(
            triangularpdf(
        ...
        self.assertAlmostEqual(
            triangularpdf(Numeric(0.5), Numeric(-1), Numeric(1), Numeric(0)).getValue(),
            0.5,
        )
    ...
    def test_mixed_inputs(self):
        # Test with mixed type inputs
        self.assertAlmostEqual(triangularpdf(0.5, Numeric(-1), 1, 0).getValue(), 0.5)
        # Add more mixed type tests here

    def test_edge_cases(self):
        # Test edge cases (x at the boundaries a, b, and c)
        self.assertAlmostEqual(triangularpdf(-1, -1, 1, 0).getValue(), 0.0)
        self.assertAlmostEqual(triangularpdf(1, -1, 1, 0).getValue(), 0.0)
        self.assertAlmostEqual(triangularpdf(0, -1, 1, 0).getValue(), 1.0)
...
class TestLogisticCdf(unittest.TestCase):
    def test_normal_values(self):
        # Test with normal values
        self.assertAlmostEqual(logisticcdf(0).getValue(), 0.5)
        self.assertAlmostEqual(logisticcdf(1, 0, 1).getValue(), 0.7310585786300049)

    def test_expression_inputs(self):
        # Test with Expression type inputs if applicable
        self.assertAlmostEqual(logisticcdf(Numeric(0)).getValue(), 0.5)
        self.assertAlmostEqual(
            logisticcdf(Numeric(1), Numeric(0), Numeric(1)).getValue(),
            0.7310585786300049,
        )
    ...
    def test_mixed_inputs(self):
        # Test with mixed type inputs
        self.assertAlmostEqual(logisticcdf(0.0, Numeric(0), 1).getValue(), 0.5)
        # Add more mixed type tests here

    def test_large_values(self):
        # Test with very large values
        self.assertAlmostEqual(logisticcdf(1000, 0, 1).getValue(), 1.0)

    def test_small_values(self):
        # Test with very small values
        self.assertAlmostEqual(logisticcdf(-1000, 0, 1).getValue(), 0.0)
