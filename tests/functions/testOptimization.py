...
class testOptimization(unittest.TestCase):
    ...
    def testSchnabelEskow2(self):
        A = np.array([[1890.3, -1705.6, -315.8, 3000.3],
    ...
    def testLineSearch(self):
        ...
        alpha, _ = opt.lineSearch(self.theFunction, x, f, g, -g)
        self.assertAlmostEqual(alpha, 0.0009765625)

    def testNewton(self):
        x0 = np.array([-1.5, 1.5])
        xstar, _ = opt.newtonLineSearch(self.theFunction, x0)
        for i in list(xstar):
            self.assertAlmostEqual(i, 1, 4)

    def testNewtonTrustRegion(self):
        x0 = np.array([-1.5, 1.5])
        xstar, _ = opt.newtonTrustRegion(self.theFunction, x0)
        for i in list(xstar):
            self.assertAlmostEqual(i, 1, 4)
    ...
    def testBioNewtonSimpleBounds(self):
        results = self.myBiogeme.estimate(algorithm=opt.simpleBoundsNewtonAlgorithmForBiogeme)
        beta = results.getBetaValues()
        self.assertAlmostEqual(beta['beta1'], 0.144546, 3)
        self.assertAlmostEqual(beta['beta2'], 0.023502, 3)
